...
%si les tailles ne sont pas les mêmes
...
[M N D] = size(hdrImage); % M = X, N = Y, D nombre de caneaux

if (nargin == 2) % verifie que la taille de l'image est superieur à la taille de la fenetre (11x11 px)
   if ((M < 11) || (N < 11))
	   s_maps = -Inf;
       s_local = -Inf;
       S = -Inf;
